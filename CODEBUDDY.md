...
5. **禁止超越函数（powi/powf/sin/cos/exp/ln 等 libm 调用）**：实现跨平台（Windows/Android）可能在末位不一致 → 失同步。模拟内只允许 `+ − × ÷` 和 `sqrt`（IEEE 754 逐比特确定）；层叠幂用显式连乘（`for _ in 0..n { x *= v }`，见 `components.rs` StatFold）
6. **任何影响模拟的改动（数值/AI/地图/洗牌/帧率）必须给 `SIM_VERSION`（`constants.rs`）+1**——录像回放靠它校验版本，不匹配会警告结果失真
